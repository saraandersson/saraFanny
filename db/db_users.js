...
class Db_user{
	...
	getUser(id, callback){
		var getSql = `SELECT users.firstname, users.lastname, users.email, users.password, users.img, users.area, users.buffert, users.consumption, users.online, user_sell_buy.sell, user_sell_buy.buy FROM users JOIN user_sell_buy ON users.id = user_sell_buy.user_id WHERE users.id = ?`;
