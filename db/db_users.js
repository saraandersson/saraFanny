...
getAllProsumers(callback){
	...
	user_production.total_excess FROM users JOIN blocked ON users.id = blocked.users_id WHERE users.id = 0 JOIN user_production ON users.id = user_production.user_id`;
